...
declare
  o boolean;
begin
  --raise notice 'check random success % %', t, successPercentage;
  o := (select setting from test_action_overrides
       where override = t);
  if o is null then
    --raise notice 'no override %', t;
    return (random() * 100) < successPercentage;
  else
    --raise notice 'override % %', t, o;
...
begin
  ...
  if (select turn_phase = 'cast' from turn_phase_table) then
    ...
    if (select count(*) from wizard_spell_choices
         inner join current_wizard_table
         on (current_wizard = wizard_name)) > 0 then
  ...
  elseif (select turn_phase = 'move' from turn_phase_table) then
    -- mounted wizards and monsters covered by blob are included
    -- but won't be selectable in the usual way:
    -- a mounted wizard can become selected via the monster he
    -- is mounted on,
    -- select from man pieces join pieces on top?
